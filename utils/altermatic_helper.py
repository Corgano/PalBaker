...
def extract_blend_metadata(blender_path: str, blend_file_path: str) -> dict:
    ...
    cmd = [
        blender_path, "-b", blend_file_path,
        "--python-expr", BLENDER_EXTRACTOR_SCRIPT
    ]
    try:
        ...
        try:
            ...
            for slot_name, mat_override_name in material_overrides.items():
                ...
                if slot_name_lower in slots_order_lower and mat_override_name:
                    ...
                    mat_replace_list.append({"Index": str(idx), "MatPath": resolved_mat_path})
        ...
        for f in os.listdir(directory):
            if f.endswith("_blend.json"):
                ...
                try:
                    ...
                    pass
    if not materials:
        materials = [f"MI_{character_id}_Body_Latex", f"MI_{character_id}_Body_Shiny", f"MI_{character_id}_Body_Gold"]
